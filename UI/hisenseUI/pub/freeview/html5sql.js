...
var html5sql = (function () {
		...
		sqlProcessor = function (transaction, sqlObjects, finalSuccess, failure) {
			...
			var sequenceNumber = 0,
				...
				successCallback = function (transaction, results) {
					...
					sequenceNumber++;
					if (dataForNextTransaction && jQuery.isArray(dataForNextTransaction)) {
			...
			for (i = 0; i < sqlInput.length; i++) {
				...
				if (typeof sqlInput[i] === "string") {
					...
					if (typeof sqlInput[i]         !== "object"   ||
						...
						!jQuery.isArray(sqlInput[i].data)) {
	...
	return {
		...
		openDatabase: function (name, version, displayname, size, whenOpen) {
			html5sql.database = openDatabase(name, version, displayname, size);
