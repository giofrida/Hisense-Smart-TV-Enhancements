...
function getSettingChSetAutoScanPageData(opt){
    opt.CaE =[
        {
            "id":"settingChSetChannelScanHeadImg",
            "description":"",
            "CaEType":"img"
        },
        {
...
var settingChSetAutoScanPageData={
    "settingChSetChannelScanHeadImg":{"Data":"img/head_arrow.png"},
    ...
    "operateData":{
        ...
        "currScanMode":0, //0:ATV,1:DTV,2:DTV+ATV
        "currScanModeNum":0
    },
    "langData":{
        "Auto Scan":["Auto Scan"],
...
function settingInitChSetAutoScanPage(){
    try{
        debugPrint("--EM-- settingInitChSetAutoScanPage",DebugLevel.ALWAYS);
        ...
        if(tv == false){
            ...
            if(data.operateData.currTunerMode == 2){
                ...
                if(data.operateData.currTunerMode == 0){  //DVBT
                    ...
                    data.operateData.currScanMode = 0;  //DVBC 下只搜ATV
                }
...
function settingRewriteChSetChScanPage(data){
    try{
        ...
        if(hiWebOsFrame.getHTMLDir() == "ltr"){
            data.settingChSetChannelScanHeadImg.Data = "img/head_arrow.png";
        }else{
            data.settingChSetChannelScanHeadImg.Data = "img/head_arrow_right.png"
        }
...
function settingChSetStartChScan(){
    try{
        ...
        if(tv == false){
            ...
            debugPrint("settingChSetStartChScan:start!currScanMode="+data.operateData.currScanMode,DebugLevel.ALWAYS);
            ...
            debugPrint("settingInitChSetAutoScanPage:currScanMode="+data.operateData.currScanMode+",currScanModeNum="+data.operateData.currScanModeNum,DebugLevel.ERROR);
        }
    }catch (ex){
...
//                settingChSetChScanToSettingPage();
            ...
            if(data.operateData.chScanState == 0){
                ...
                data.operateData.chScanState = 1;
                hiWebOsFrame.ChSetChannelScanPage.rewriteDataOnly();
                if(data.operateData.currTunerMode == 0 && hiWebOsFrame.getCurrentCountry() == "Malaysia"){
...
function settingChSetChScanToBlankPage(){
    try{
        ...
        //hiWebOsFrame.ChSetChannelScanPage.destroy();
        closePagesOrModuleByPage(hiWebOsFrame.ChSetChannelScanPage); //quick page 需要重新创建
    }catch (ex){
        ...
        switch (value){
            case 1://complete
                if(data.operateData.currScanMode == 2 && data.operateData.currScanModeNum < 2){
                    ...
                    data.operateData.chScanState = 1;
//                    data.operateData.totalChannelNum = model.channelSearch.getFoundTotalServices();
//                    data.operateData.analogChannelNum = model.channelSearch.getFoundAnalogServices();
//                    data.operateData.digitChannelNum = model.channelSearch.getFoundDigitServices();
//                    data.operateData.radioNum = model.channelSearch.getFoundRadioServices();
//                    data.operateData.totalChannelNum = data.operateData.analogChannelNum+data.operateData.digitChannelNum;
                    hiWebOsFrame.ChSetChannelScanPage.rewriteDataOnly();
...
function settingChSetAutoScanPageOnOpen(){
    try{
        var data = settingChSetAutoScanPageData;
        if(data.operateData.currTunerMode == 1 ){  //DVBC 下不显示DTV台个数
            $("#settingChSetChScanDigitalBtn").css("display","none");
        }
        if(tv == true){
            if(model.channelSearch.getRunning() == 1){
