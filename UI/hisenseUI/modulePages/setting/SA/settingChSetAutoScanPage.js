...
function getSettingChSetAutoScanPageData(opt){
    opt.CaE =[
        ...
        {
            "id":"settingChSetAutoScanFreqTitle",
            "description":"",
            "CaEType":"span"
        },
        {
            "id":"settingChSetAutoScanFreqValue",
            "description":"",
            "CaEType":"span"
        },
        {
...
var settingChSetAutoScanPageData={
    ...
    "settingChSetAutoScanTotalTitle":{"Data":"Channels Found"},
    "settingChSetAutoScanTotalValue":{"Data":5},
    "settingChSetAutoScanFreqTitle":{"Data":"Frequency:"},
    "settingChSetAutoScanFreqValue":{"Data":5},
    ...
    "settingChSetChScanProgressTitle":{"Data":"Complete"},
    ...
    "operateData":{
        ...
        "currTunerMode":1,//0:DVBT,1:DVBC,2:DVBS
        "currChNumber":"",
        "currPercent":0,
        "totalChannelNum":0,
        "currScanFreq":0,
...
function settingInitChSetAutoScanPage(){
    try{
        debugPrint("SA settingInitChSetAutoScanPage:",DebugLevel.ALWAYS);
        var data = settingChSetAutoScanPageData;
        data.operateData.totalChannelNum = 0;
        data.operateData.currScanFreq = 0;
...
function settingRewriteChSetChScanPage(data){
    try{
        ...
        data.settingChSetAutoScanFreqValue.Data = data.operateData.currScanFreq;
...
function settingChSetStartChScan(){
    try{
        ...
        if(tv == false){
            ...
            debugPrint("settingChSetStartChScan:start scan!",DebugLevel.ALWAYS);
            model.channelSearch.Start();
        }
    }catch (ex){
...
function testProgressBar(){
    ...
    data.operateData.totalChannelNum = data.operateData.digitChannelNum + data.operateData.analogChannelNum;
...
function settingChSetStopChScan(){
    try{
        ...
        if(tv == false){
            ...
            if(data.operateData.chScanState == 0){
                debugPrint("wizardChScanStopScan:"+data.operateData.chScanState);
                //正在搜索,取消搜索
                model.channelSearch.Stop();
                data.operateData.chScanState = 1;
                hiWebOsFrame.ChSetChannelScanPage.rewriteDataOnly();
                debugPrint("wizardChScanStopScan stop!!!!!!!!!!!!!!!");
            }else{
...
function settingChScanProgressChangeCallBack(value){
    try{
        var data = settingChSetAutoScanPageData;
        debugPrint("settingChScanProgressChangeCallBack:"+"percent:"+value,DebugLevel.ERROR);
...
/*******************************************************************
 name:settingChScanFrequencyChangeCallBack
 description:监控搜索频点的变化
 input:state:当前搜索到的数字信号的个数
 output:
 return
 *******************************************************************/
function settingChScanFrequencyChangeCallBack(value){
    try{
        var data = settingChSetAutoScanPageData;
        debugPrint("settingChScanFrequencyChangeCallBack:currScanFreq="+value);
        data.operateData.currScanFreq = value;
        hiWebOsFrame.ChSetChannelScanPage.rewriteDataOnly();
    }catch(ex){
        debugPrint("settingChScanFrequencyChangeCallBack:"+ex.message,DebugLevel.ERROR);
    }
}
/*******************************************************************
...
function settingChScanStateChangeCallBack(value){
    try{
        var data = settingChSetAutoScanPageData;
        debugPrint("SA settingChScanStateChangeCallBack:state="+value,DebugLevel.ALWAYS);
        switch (value){
            case 1://complete
                data.operateData.chScanState = 1;
...
function settingChSetAutoScanPageOnOpen(){
    try{
        var data = settingChSetAutoScanPageData;
        if(data.operateData.currTunerMode == 1){
            $("#settingChSetChScanDigitalTitle").parent().css("display","none")
        }else{
            $("#settingChSetChScanDigitalTitle").parent().css("display","block")
        }
        if(tv == true){
            if(model.channelSearch.getRunning() == 1){
...
//            model.source.InputSet(0);
            ...
            model.channelSearch.onFoundServicesFrequencyChaged = settingChScanFrequencyChangeCallBack;
