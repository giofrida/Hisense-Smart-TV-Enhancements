...
{
//    PageDateQuickSet.operateData.headphone = defaultHeadphone;
    if(!tv)
    {
        PageDateQuickSet.operateData.daItem = [];
        PageDateQuickSet.operateData.nav = defaultNav;
        var audioTrack_audioLanguage_subtitle_Disable = [];
...
//        if (1 == 1)
	    {   //当前为LiveTV模式
		    ...
		    {   //当前有信号
			    try{
				    PageDateQuickSet.operateData.teletextIndex = model.system.getCurChannelTeletextSelect();
				    debugRM("getTeletextIndex :" + PageDateQuickSet.operateData.teletextIndex);
			    }catch (e){
				    debugE(e.message);
			    }
			    ...
			    {   //当前为ATV
				    ...
				    debugRM("currentChannel:ATV. Disable:Audio Language & Subtitle...Received value:" + parseInt(livetvmain.getCurrentChannelInfo().type));
				    var audioLanguage_subtitle_Disable = [1,2];
				    debugRM("Signal:Yes...Received value:" + livetvmain.getNoSignalFlag());
				    if(ifCurrentIn3DMode) {
					    audioLanguage_subtitle_Disable = [1,2,4];
				    }
			    ...
			    {   //当前为DTV
				    ...
				    debugRM("currentChannel:DTV. Disable:None...Received value:" + parseInt(livetvmain.getCurrentChannelInfo().type));
				    var audioTrack_Disable = [0];
				    debugRM("Signal:Yes...Received value:" + livetvmain.getNoSignalFlag());
				    if(ifCurrentIn3DMode) {
					    audioTrack_Disable = [0,4];
				    }
...
//    var headphoneInsertState =  1;
	...
	if (!tv) {
		...
		try {
			...
			PageDateQuickSet.operateData.Support3d = (1 == model.video.get3dSupported() && 0 != model.video.getEnum3dMode());
			debugE("Support3d !!!!!!!!!!!!!!!!!!" + PageDateQuickSet.operateData.Support3d);
			if (PageDateQuickSet.operateData.Support3d) {
				if (PageDateQuickSet.operateData.nav.length == 1) {
					PageDateQuickSet.operateData.daItem = [0];
				}
				else {
					var index = _getIndex(PageDateQuickSet.operateData.daItem, 4);
					if (index < 0) {
						PageDateQuickSet.operateData.daItem.push(4);
					}
				}
			}
...
function SettingQsTopFiveEnHandler()
{
    ...
    else
    {
		switch (ind) {
			case 0:
				PageDateQuickSet.operateData.cursubselect="audioTrack";
			...
			case 2:
				...
				break;
			case 3:
//				PageDateQuickSet.operateData.cursubselect="Teletext";
//				hiWebOsFrame.createPage('setting_sys_qs_list', null, hiWebOsFrame.settingsysqS, null, function (qslist) {
//					hiWebOsFrame.settingsysqS.close();
//					hiWebOsFrame.settingqslist = qslist;
//					hiWebOsFrame.settingqslist.open();
//					hiWebOsFrame.settingqslist.hiFocus();
//				});
//				if(0 == PageDateQuickSet.operateData.teletextIndex)
//				{
//					try{
//						model.system.setCurChannelTeletextSelect(1);
//						debugRM("setTeletextIndex :" + 1);
//					}catch (e){
//						debugE(e.message);
//					}
//					PageDateQuickSet.operateData.teletextIndex = 1;
//					this.page.close();
//					this.page.destroy();
//					hiWebOsFrame.settingsysqS.origin.hiFocus();
//				}
//				else if(1 == PageDateQuickSet.operateData.teletextIndex)
//				{
//					try{
//						model.system.setCurChannelTeletextSelect(0);
//						debugRM("setTeletextIndex :" + 0);
//					}catch (e){
//						debugE(e.message);
//					}
//					PageDateQuickSet.operateData.teletextIndex = 0;
//				}
                this.page.close();
                //this.page.destroy();
                if(hiWebOsFrame.settingsysqS.origin.module=="livetv")
                {
			...
			switch (index) {
				case 0:  //picMode
					...
					hiWebOsFrame.createPage('setting_pic_mode', null, hiWebOsFrame.settingsFirst, null, function (SettingPicModePage) {
						...
						//var helpInfo = PageDataPicModeData.operateData.helpInfo;
						//CreateSndHelpInfoPage(helpInfo.title, helpInfo.content, false);
						//var pos = sndHelpInfo.getPosZIndex('setting_pic_mode');
						//sndHelpInfo.setHelpInfoPosZIndex(pos.top - 175, pos.left, pos.width, sndHelpInfo.defaultHeight, pos.zIndex + 1);
				...
				case 5: //图像比例
					...
					hiWebOsFrame.createPage('setting_pic_aspect_ratio', null, hiWebOsFrame.settingsFirst, null, function (SettingPicSizePage) {
						...
						//var helpInfo = PageDataPicAspectRatioData.operateData.helpInfo;
						//CreateSndHelpInfoPage(helpInfo.title, helpInfo.content, false);
			...
			else
			{
				...
				hiWebOsFrame.createPage('setting_sys_lang1_page', null, hiWebOsFrame.settingsFirst, null, function (lang1page) {
					...
					hiWebOsFrame.createPage('setting_sys_lang2_page', null, hiWebOsFrame.settingslang1, null, function (lang2page) {
						hiWebOsFrame.settingslang2 = lang2page;
						lang2page.close();
                        if(tmp=="setting_sys_lang_audio3")
                        {
                            SettingLangUpdateHelpinfo(PageDataSettingSysLang.operateData.helplist[1].content);

                        }else
                        {
                            SettingLangUpdateHelpinfo(PageDataSettingSysLang.operateData.helplist[3].content);
