...
//    var itemCount = 0;
    ...
    function rewritePageData(pageData) {
        ...
        for (var i = 0; i < oprData.channelList.length; i++) {
            ...
            if(i < oprData.channelList.length -2){
                ...
                if(i == oprData.channelList.length - 1){
                    pageData.chl_manager_list_edit.Data.push({
                        chl_manager_list_edit_item: {
                            Data: 'Import'
                        }
...
//        var pages = Math.ceil(rows.length / 7);
        var convertList = [],convertListNoUSB = [];
...
//            }
        ...
        oprData.NoUSBchannellist = convertListNoUSB;
        convertList.push({
            name:"Export to USB",
            uid: "",
            satId: "",
    ...
    self.onOpenChlManager = function () {
        ...
        $("#chl_manager_list_display").css("display","none");
    }
...
//                    channelmanagerfav.setRightSelectedIndex(hiWebOsFrame.channel_manager_fav_edit.getCaE("chl_manager_list_type2"));
        ...
        }else if(this.SelectedIndex >=oprData.channelList.length - 2 ){
          if(this.SelectedIndex ==oprData.channelList.length - 2){ //进入export to usb 页面
              try {
                  hiWebOsFrame.createPage('setting_chnl_advanced_export_to_usb_dialog', null, null, null, function (SettingChnlExportToUSBPage) {
                      ...
                      SettingChnlExportToUSBPage.origin = hiWebOsFrame.channel_manager_page;
              ...
              try {
                  hiWebOsFrame.createPage('setting_chnl_advanced_import_from_usb_dialog', null, null, null, function (SettingChnlImportFromUSBPage) {
                      ...
                      SettingChnlImportFromUSBPage.origin = hiWebOsFrame.channel_manager_page;
...
//                a.hiFocus();
    ...
    self.getLists = function(){
        return  oprData.NoUSBchannellist;
    }
    self.setSelectedIndex = function (comp) {
        comp.setSelectedIndex(0);
    }
    function eventToChannelList(rows) {
