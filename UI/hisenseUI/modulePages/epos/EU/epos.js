...
function slideshowBackup() {
    debugPrint('slideshowBackup is called! BackgroundIamge is showing......now index is ' + eposPageData.operateData.slide);
...
function eposPageonOpen() {
    ...
    //   initText();
    //隐藏第2~4个panel
    eposPageData.operateData.eposIndex = 0;
    if (!!eposPageData.operateData.eposInterval) {
        clearInterval(eposPageData.operateData.eposInterval);
    }
    eposPageData.operateData.slideshowTimer_eu = setInterval(function () {
        debugPrint(eposPageData.operateData.eposIndex + "-------------eposPageData.operateData.eposIndex");
        //var nowIndex = eposIndex / 8;
        switch (eposPageData.operateData.eposIndex) {
            case 0:
                $("#epos1").css("opacity", 1);
                break;
            case 3:
        ...
        if (eposPageData.operateData.eposIndex == 11) {
            eposPageData.operateData.eposIndex = 0
        } else {
            eposPageData.operateData.eposIndex++;
        }
    }, 5000);
...
function eposonClose() {
    ...
    clearInterval(eposPageData.operateData.slideshowTimer_eu);
    eposPageData.operateData.slideshowTimer_eu = null;
...
function anyKeyHandler() {
    debugPrint("anyKeyHandler is calling....getKeyStackTop:.." + hiWebOsFrame.getKeyStackTop());

        switch (hiWebOsFrame.getKeyStackTop()) {
            case VK_VOLUME_DOWN:
...
function playInsideMovie() {
    ...
    if (serviceNo.indexOf("K321") != -1 || serviceNo.indexOf("K700") != -1) {
        insideMoviePath = "file:///3rd/4kvideo.mkv";
    }
 ...
 * 说明：
 *   目前有四个机型，XT910,K700,K720,K321,默认显示为K321内容。
 * */
function initEPOSImgByServiceNo() {
    ...
    //serviceNo = "K700";
    //LTDN65K700XWAU3D
    if (serviceNo.indexOf("K700") != -1) {
        DBG_ALWAYS("now K700!!!!");
        $("#epos1").attr("src", "img/epos/EU/K700epos-1.png");
        $("#epos2").attr("src", "img/epos/EU/K700epos-2.png");
        $("#epos3").attr("src", "img/epos/EU/K700epos-3.png");


    } else if (serviceNo.indexOf("K720") != -1) {
        DBG_ALWAYS("now K720!!!!");
        $("#epos1").attr("src", "img/epos/EU/K720epos-1.png");
        $("#epos2").attr("src", "img/epos/EU/K720epos-2.png");
        $("#epos3").attr("src", "img/epos/EU/K720epos-3.png");
    } else if (serviceNo.indexOf("XT910") != -1) {
        DBG_ALWAYS("now K910!!!!");
        $("#epos1").attr("src", "img/epos/EU/XT910epos-1.png");
        $("#epos2").attr("src", "img/epos/EU/XT910epos-2.png");
        $("#epos3").attr("src", "img/epos/EU/XT910epos-3.png");
    }
    else {
        DBG_ALWAYS("now K321!!!!");
        $("#epos1").attr("src", "img/epos/EU/epos1.png");
        $("#epos2").attr("src", "img/epos/EU/epos2.png");
        $("#epos3").attr("src", "img/epos/EU/epos3.png");
    }
}
