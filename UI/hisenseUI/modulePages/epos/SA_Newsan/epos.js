...
var eposPageData = {
    ...
    rewrite: function (page) {
        ...
        switch (langIdx) {
            case 0://英语
                lanStr = "eng";
                opData.text_uhd = ["Quality", "Enjoy flawless moving images with brilliant colors and realistic picture detail", "Enhance viewing with improved sound quality and performance"];
                opData.text_picture = ["Apps", "Browse and enjoy thousand of titles and streaming content. Enjoy movies,shows, and music with the click of a button"];
                opData.text_smarttv = ["Smart", "Rich content & easy to use", "Web browser, App store, Anyview Cast"];
                opData.text_connect = ["Connectivity", "Dual band</br> Wireless Network", "Quad-core", "USB Video", "HDMI"];
                break;

            case 2://西班牙
                lanStr = "spa";
                opData.text_uhd = ["Calidad", "Disfrute de las imágenes en movimiento</br> con colores brillantes y realistas e detalles</br> realistas de imagen", "Mejore la visualización con una mejor calidad y rendimiento de sonido"];
                opData.text_picture = ["Apps", "Explore y disfrute de miles de títulos y contenidos en streaming.Disfrute de películas, espectáculos y música con el clic de un botón."];
                opData.text_smarttv = ["Inteligente", "Contenido rico y fácil de usar", "Navegador web, App store, Anyview Cast"];
                opData.text_connect = ["Conectividad", "Red inalámbrica </br>de banda dual", "Quad-core", "vídeo USB", "HDMI"];
                break;
            case 3://葡萄牙
                lanStr = "por";
                opData.text_uhd = ["Qualidade", "Desfrute de imagens em movimento sem</br> falhas com cores brilhantes e detalhes de </br>imagem realista", "Melhorar a ver com a melhoria da qualidade e desempenho de som"];
                opData.text_picture = ["Apps", "Navegue e desfrutar de milhares de títulos e streaming de conteúdo.Desfrute de filmes, shows e música com o clique de um botão"];
                opData.text_smarttv = ["Smart", "Conteúdo rico e fácil de usar", "Navegador de Web,Loja de Apps,Anyview Cast"];
                opData.text_connect = ["Conectividade", "Banda dupla</br>de rede sem fio", "Quad-core", "vídeo USB", "HDMI"];
                break;
            default:
                lanStr = "eng";
                opData.text_uhd = ["Quality", "Enjoy flawless moving images with brilliant colors and realistic picture detail", "Enhance viewing with improved sound quality and performance"];
...
function changeOEMLOGO() {
    var brand = 'his';
    try {
        brand = model.system.getCurBrand();
    ...
    switch (brand) {
        ...
        case 'nob':
            ...
            var pngurl = "img/epos/SA_Newsan/" + brand + ".png";
            $('#logoImg').attr('src', pngurl);
            break;
        default :
            DBG_ERROR("Error : wrong LOGO " + brand);
            $('#logo').css("display", "block");
            $('#logoImg').attr('src', "img/epos/SA_Newsan/smart.png");
            break;
    }
}
...
function changeEPOSContent() {
    ...
    if ("APP_5890_SA" == currentPlatform) {
        $("#table_uhd").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_uhd_panel_5657.png)");
        switch (hiWebOsFrame.getCurrentCountry()) {
            case "Argentina":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_1_5657.png)");
                break;
            case "Brazil":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_2_5657.png)");
                break;
            case "Philippines":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_3_5657.png)");
                break;
            case "Uruguay":
            case "Colombia":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_4_5657.png)");
                break;
            default :
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_5_5657.png)");
                break;
        }
    } else {
        $("#table_uhd").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_uhd_panel.png)");
        debugPrint("changeEPOSContent begin! currentPlatform is " + hiWebOsFrame.getCurrentCountry());
        switch (hiWebOsFrame.getCurrentCountry()) {
            case "Argentina":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_1.png)");
                break;
            case "Brazil":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_2.png)");
                break;
            case "Philippines":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_3.png)");
                break;
            case "Uruguay":
            case "Colombia":
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_4.png)");
                break;
            default :
                $("#table_picture").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_picture_panel_5.png)");
                break;
        }
    }
    //根据国家切换APP内容
    ...
    try {
        ...
        DBG_ALWAYS("Epos resolution is " + EPOSresolution + "MEMC_SUPPORT is " + MEMC_SUPPORT);
    ...
    if ("HIS_EPOS_HD" == EPOSresolution) {
        if (MEMC_SUPPORT == 1) {
            $("#table_uhd").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_uhd_panel_led.png)");

        } else {
            $("#table_uhd").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_uhd_panel_led_nomemc.png)");
        }
        var langIdx = 0;
        try {
            langIdx = model.language.getOsd();
        ...
        switch (langIdx) {
            case 0://英语
                $("#txt_connect1").html("Built in Wireless Network");
                break;
            case 2://西班牙
                $("#txt_connect1").html("Construido en la red inalámbrica");
                break;
            case 3://葡萄牙
                $("#txt_connect1").html("Construído em rede sem fio");
                break;
            default :
                $("#txt_connect1").html("Built in Wireless Network");
                break;
        ...
        if (MEMC_SUPPORT == 1) {
            ...
            $("#table_uhd").css("background-image", "url(img/epos/SA_Newsan/uhd_epos_uhd_panel_nomemc.png)");
        }
        var langIdx = 0;
        try {
            langIdx = model.language.getOsd();
