...
function Keyboard() {
    ...
    var onKey = function (key) {
        ...
        if (g_SystemAudioOnlyFlag == 1 && (keyCode != VK_MUTE && keyCode != VK_VOLUME_DOWN && keyCode != VK_VOLUME_UP)) {

            try {
                if (isRepeated || isKeyUp) {
                    //key.sendResult(true);
                    sendResult(key, true);
                }
                else {
        ...
        try {
            ...
            if (!key_hasProcessFlag) {
                ...
                if (typeof  (GlobalFlagShareInBrowser) != UNDEFINED_DEFSTR && ((4 == GlobalFlagShareInBrowser) || (13 == GlobalFlagShareInBrowser)) && keyCode == VK_BACK) {
                    ...
                    sendResult(key, false); //专门针对Picasa页面还有HiMedia中的DROPbox
...
//        debugPrint("the key"+JSON.stringify(currkey.command));
        currkey.sendResult(value);
