...
function Keyboard() {
    ...
    var KEYBOARD_KEYCODE_SCANCODE_MAP = {};
    {
        ...
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_LIVETV] = VK_LIVETV;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_CH_LIST] = VK_CH_LIST;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_FAV_CH] = VK_FAV_CH;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_CINEMA_NIGHT] = VK_PIC_CINEMA_NIGHT;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_CINEMA_DAY] = VK_PIC_CINEMA_DAY;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_RATIO_AUTO] = VK_ASPECT_AUTO;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_RATIO_DIRECT] = VK_ASPECT_DIRECT;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_S_MOVIE] = VK_SND_MOVIE;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_FREEZE] = VK_FREEZE;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_MENU] = VK_MENU;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_SECMENU] = VK_MENU;
        ...
        //add by ghl------------------
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_MENU7] = VK_KEYPAD_MENU7;
        ...
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_AUDIO_DESCRIPTION] = VK_AUDIO_DESCRIPTION;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_WUAKI] = VK_WUAKI;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_LOW_BATTERY] = VK_LOWBATTERY;

        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_POWER_ON] = VK_POWER_ON;

        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_POWERKEYPAD] = VK_POWER_KEY_PAD;
        SYSCMD_KEYCODE_MAP[guijs.SYSCMD_AUDIOONLY] = VK_AUDIOONLY;
    ...
    var onKey = function (key) {
        ...
        if (VK_POWER_ON == SYSCMD_KEYCODE_MAP[key.command]) {
            if (waitSuspendPowerOn) {
                waitSuspendPowerOn = false;
                recoverAppState();
            }
            sendResult(key, true);
            return;
        ...
        if (isKeyDown) {
            if (m_retailmodeTimer && keyCode != VK_VOLUME_DOWN && keyCode != VK_VOLUME_UP && keyCode != VK_MUTE) {
                ...
                try {
                    ...
                    if (data == "open") {//running, stop ePos
                        if (!isEUXT910() && !!hiWebOsFrame.getPageByIdFromSdkPages("epos")) {
                            debugPrint("close epos!");
                            startePos(false);
        ...
        if (isKeyUp) {
            ...
            if (g_systemautosleepflag > 0) {
                clearTimeout(g_systemautosleepTimer);
                g_systemautosleepTimer = null;
                var sleepvalue = [0, 3, 4];
                var timer = 4;
                if (g_systemautosleepflag < 3) {
                    timer = sleepvalue[g_systemautosleepflag];
                }
                //debugPrint("@@@@@@@@@@@@@@@@@@@@@@@@@")
        ...
        else if (g_SystemAudioOnlyFlag == 2) {
            isKeyUp && (g_SystemAudioOnlyFlag = 0);
            sendResult(key, true);
            return;
        }
    ...
    this.dynamicDispatchKeyEvent = function (type, keyCode, from) {
        dispatchKeyEvent(type, keyCode, from);
    }
    ...
    this.set_mheg5_active = function (is_mhge5_active) {
